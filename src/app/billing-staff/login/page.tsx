...
const billingLoginSchema = z.object({
  identifier: z
    .string()
    .min(3, "Please enter your finance email or cashier ID")
    .refine((val) => {
      if (val.includes("@")) {
...
type BillingLoginFormValues = z.infer<typeof billingLoginSchema>;

export default function BillingStaffLoginPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<BillingLoginFormValues>({
    resolver: zodResolver(billingLoginSchema),
    defaultValues: {
      identifier: "",
      password: "",
    },
  ...
  const onSubmit = async (values: BillingLoginFormValues) => {
    ...
    try {
      ...
      if (!res.data?.success || !res.data?.data?.user) {
        throw new Error(res.data?.error?.message || "Invalid billing staff credentials.");
      }
      ...
      router.push("/billing-staff");
    } catch (err: any) {
      const msg =
        err?.response?.data?.error?.message ||
        err?.message ||
        "Invalid credentials. Please verify your finance email and password.";
      setErrorMsg(msg);
    } finally {
    ...
    <div className="min-h-screen bg-surface-container-lowest flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h1 className="text-2xl font-black text-on-surface tracking-tight">
          Billing & Cashier Login
        </h1>
        <p className="text-xs text-outline mt-1 max-w-sm mx-auto">
          Patient invoicing, co-pay receipts, claim reconciliations, and cash drawer management.
        </p>
      </div>
        ...
        <div className="bg-surface-container-lowest py-8 px-6 shadow-xl rounded-3xl sm:px-10 border border-outline-variant/40">
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Button
              type="submit"
              variant="primary"
              className="w-full h-11 font-bold rounded-xl mt-2 bg-violet-700 hover:bg-violet-800 text-white"
              disabled={isSubmitting}
            >
              {isSubmitting ? "Signing in..." : "Open Billing Desk"}
            </Button>
          </form>

          <div className="mt-6 pt-6 border-t border-outline-variant/20 text-center">
            <p className="text-xs text-outline">
              New cashier or billing staff?{" "}
              <Link
                href="/billing-staff/register"
