...
const nurseLoginSchema = z.object({
  identifier: z
    .string()
    .min(3, "Please enter your hospital email or nurse staff ID")
    .refine((val) => {
      if (val.includes("@")) {
...
type NurseLoginFormValues = z.infer<typeof nurseLoginSchema>;
...
export default function NurseLoginPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<NurseLoginFormValues>({
    resolver: zodResolver(nurseLoginSchema),
    defaultValues: {
      identifier: "",
      password: "",
    },
  ...
  const onSubmit = async (values: NurseLoginFormValues) => {
    ...
    try {
      ...
      if (!res.data?.success || !res.data?.data?.user) {
        throw new Error(res.data?.error?.message || "Invalid nurse credentials.");
      }
      ...
      const msg =
        ...
        err?.message ||
        "Invalid credentials. Please verify your hospital email and password.";
      setErrorMsg(msg);
    } finally {
    ...
    <div className="min-h-screen bg-surface-container-lowest flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h1 className="text-2xl font-black text-on-surface tracking-tight">
          Nursing Station Login
        </h1>
        <p className="text-xs text-outline mt-1 max-w-sm mx-auto">
          Inpatient ward triage, bed allocations, vitals charting, and medication administration.
        </p>
      </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              <label className="block text-xs font-bold text-on-surface uppercase tracking-wider mb-1">
                Hospital Work Email or Staff ID
              </label>
              <input
                type="text"
                {...register("identifier")}
                placeholder="nurse.vance@goingmerry.hms"
