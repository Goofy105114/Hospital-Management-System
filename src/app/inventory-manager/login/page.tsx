...
const inventoryLoginSchema = z.object({
  identifier: z
    .string()
    .min(3, "Please enter your supply chain email or depot ID")
    .refine((val) => {
      if (val.includes("@")) {
...
type InventoryLoginFormValues = z.infer<typeof inventoryLoginSchema>;
...
export default function InventoryLoginPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<InventoryLoginFormValues>({
    resolver: zodResolver(inventoryLoginSchema),
    defaultValues: {
      identifier: "",
      password: "",
    },
  ...
  const onSubmit = async (values: InventoryLoginFormValues) => {
    ...
    try {
      ...
      if (!res.data?.success || !res.data?.data?.user) {
        throw new Error(res.data?.error?.message || "Invalid inventory manager credentials.");
      }
      ...
      const msg =
        ...
        err?.message ||
        "Invalid credentials. Please verify your logistics email and password.";
      setErrorMsg(msg);
    } finally {
    ...
    <div className="min-h-screen bg-surface-container-lowest flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h1 className="text-2xl font-black text-on-surface tracking-tight">
          Supply & Inventory Login
        </h1>
        <p className="text-xs text-outline mt-1 max-w-sm mx-auto">
          Central logistics, warehouse purchase orders, batch expiration tracking, and stock transfers.
        </p>
      </div>
        ...
        <div className="bg-surface-container-lowest py-8 px-6 shadow-xl rounded-3xl sm:px-10 border border-outline-variant/40">
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              <label className="block text-xs font-bold text-on-surface uppercase tracking-wider mb-1">
                Logistics Email or Depot Staff ID
              </label>
              <input
                type="text"
                {...register("identifier")}
                placeholder="inventory@goingmerry.hms"
            ...
            <Button
              type="submit"
              variant="primary"
              className="w-full h-11 font-bold rounded-xl mt-2 bg-orange-700 hover:bg-orange-800 text-white"
              disabled={isSubmitting}
            >
              {isSubmitting ? "Signing in..." : "Access Inventory Console"}
            </Button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-outline-variant/20 text-center">
            <p className="text-xs text-outline">
              ...
              <Link
                href="/inventory-manager/register"
                className="font-bold text-orange-700 hover:underline"
              >
                Register Inventory Account
              </Link>
            </p>
          </div>
        </div>
