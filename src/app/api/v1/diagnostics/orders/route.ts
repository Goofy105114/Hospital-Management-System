...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      order = await prisma.diagnosticOrder.create({
        data: {
          orderNumber,
          patientId,
      ...
      // DB offline — return structured intent confirmation
      order = {
        id: `dia-${Date.now()}`,
        orderNumber: `DIA-OFFLINE-${Date.now()}`,
        patientId,
        encounterId: encounterId ?? null,
        doctorId: user.sub,
        status: "ORDERED",
        notes: notes ?? null,
        items: testIds.map((id: string) => ({ testId: id })),
      };
    }
