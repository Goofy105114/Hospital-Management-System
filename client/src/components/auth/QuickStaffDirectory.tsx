...
import { DemoUser } from '../../types/auth.js';
...
export const QuickStaffDirectory: React.FC<QuickStaffDirectoryProps> = ({
  ...
  const getPasswordForRole = (u: DemoUser): string => {
    ...
    switch (u.role) {
      ...
      case 'INVENTORY_MANAGER': return 'Inventory@123';
  ...
  const filteredUsers = demoUsers.filter((u) => {
    ...
    if (selectedFilter === 'ADMIN') return ['ADMIN', 'RECEPTIONIST', 'PHARMACIST'].includes(u.role) && u.status === 'ACTIVE';
    if (selectedFilter === 'EDGE_CASES') return u.status !== 'ACTIVE';
    return true;
  });

  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
            ...
            filteredUsers.map((u) => {
              ...
              return (
                <div
                  key={u.id}
                  onClick={() => {
                    onSelectPersona(u.email || u.phone, getPasswordForRole(u));
                    onClose();
                  }}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400 mt-0.5">
                        Role: <span className="font-semibold text-slate-600">{u.role}</span> • Password: <code className="text-slate-600 font-mono bg-slate-100 px-1 py-0.5 rounded">{getPasswordForRole(u)}</code>
                      </div>
                      {u.notes && (
