...
import { SecurityEvent, AuditLog } from '../types/auth.js';
...
export const SecurityInspector: React.FC<SecurityInspectorProps> = ({
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const data = await authApi.getSecurityOverview();
                  ...
                  >
                    ...
                    {evt.ip_address && (
                      <div className="text-[11px] text-slate-400">
                        IP: {evt.ip_address} • Agent: {evt.user_agent ? evt.user_agent.substring(0, 40) : 'N/A'}
                      </div>
                    )}
                    {evt.metadata && (
              ...
              <div className="space-y-2.5">
                {auditLogs.map((log) => (
                  ...
                  >
                    ...
                    <div className="text-slate-700">
                      Actor: <span className="font-semibold text-slate-900">{log.actor_role || 'SYSTEM'}</span> (ID: {log.actor_id || 'system'})
                    </div>
                    <div className="text-[11px] text-slate-500">
                      Entity: <span className="font-mono">{log.entity_type}</span> • ID: <span className="font-mono">{log.entity_id}</span>
                    </div>
                    {log.changes && (
                      <pre className="text-[10px] text-slate-600 bg-white p-2 rounded-lg border border-slate-200 font-mono overflow-x-auto">
                        {log.changes}
